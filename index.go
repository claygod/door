...
func (x *index) find(url []byte, mtd []byte) *route {
	salt := x.genSalt(mtd)
	...
	} else if x.tree[x.genUint([]byte{DELIMITER_SLASH}, salt)] != nil {
		cNode = x.tree[x.genUint([]byte{DELIMITER_SLASH}, salt)]
	} else {
		return nil
	}
	// slash
	...
	for _, route := range routes {
		...
		if x.tree[cHash] != nil {
			...
			case TYPE_STAT:
				x.tree[cHash] = cNode
			case TYPE_ARG:
				x.tree[x.genUint([]byte{DELIMITER_SLASH}, salt)] = cNode
			}
		}
		for i := 0; i < length; i++ {
			if i == length-1 {
				...
				case TYPE_ARG:
					if i == 0 {
						cNode.child[x.genUint([]byte{DELIMITER_SLASH}, salt)] = nNode
					} else {
...
func (x *index) genUintSlice(s []byte, total typeHash, cHashes *[HTTP_SECTION_COUNT]typeHash) int {
	c := DELIMITER_SLASH
	na := 0
	length := len(s)
	if length == 0 {
...
func (x *index) genUint(s []byte, total typeHash) typeHash {
	length := len(s)
	for i := 0; i < length; i++ {
		total = total<<5 + typeHash(s[i])
	}
...
func (x *index) genSalt(s []byte) typeHash {
